...
/**
 * 物理地址和标志
 * 64位地址：由于内存页4K对齐，低12位用于标志位，高52位作为数据位
 */
...
typedef struct s_MSADSC{
	list_h_t md_list;		//16B 链表
	spinlock_t md_lock; 	//4B  保护自身的自旋锁
	msadflgs_t md_indxflgs;	//4B  内存空间地址描述符标志
	phyadrflgs_t md_phyadrs;//8B  物理地址和标志
	void* md_odlink;		//8B  相邻且相同大小msadsc的指针
