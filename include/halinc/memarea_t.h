...
typedef struct s_BAFHLST{
	...
	uint_t af_oderpnr;		//oder对应的页面数，oder为2那就是1<<2=4
	uint_t af_fobjnr;		//多少个空闲的msadsc_t结构，即空闲页面
	uint_t af_mobjnr;		//当前结构体的msadsc_t结构总数，也就是总页面数
	uint_t af_alcindx;		//此结构的分配计数
	uint_t af_freindx;		//此结构的释放计数
	list_h_t af_frelst;		//挂载此结构的空闲msadsc_t结构
	list_h_t af_alclst;		//挂载此结构已分配的msadsc_t结构
	list_h_t af_ovelst;
}bafhlst_t;
