...
#define GDTMAX 10 		//10个GDT描述符
#define IDTMAX 256		//256个中断门描述符
...
/* GDT */
/* 描述符索引 */
#define	PRIVILEGE_KRNL	0 		//内核权限
#define	PRIVILEGE_TASK	1 		//任务权限
#define	PRIVILEGE_USER	3 		//用户权限
...
typedef struct s_GDTPTR{
	u16_t gdtLen;			//全局段描述符表长度
	u64_t gdtbass;			//全局段描述符表基地址
}__attribute__((packed)) igdtr_t;

//中断门描述符表寄存器
typedef struct s_IDTPTR{
	u16_t idtLen;			//中断门描述符表长度
	u64_t idtbass;			//中断门描述符表基地址
