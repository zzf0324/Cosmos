...
typedef struct s_PHYMMARGE{
    spinlock_t pmr_lock;    //保护这个结构的自旋锁
    u32_t pmr_type;         //内存空间类型
    u32_t pmr_stype;        //
    u32_t pmr_dtype;        //内存地址空间子类型，见上面的宏
    u32_t pmr_flgs;         //结构的标志与状态
    u32_t pmr_stus;         //内存空间状态
    u64_t pmr_saddr;        //内存空间的开始地址
    u64_t pmr_lsize;        //内存空间的大小
    u64_t pmr_end;          //内存空间的结束地址
    u64_t pmr_rrvmsaddr;    //内存保留空间的开始地址：reserved memory start address
    u64_t pmr_rrvmend;      //内存保留空间的结束地址：reserved memory end
    void* pmr_prip;         //结构的私有数据指针，以后拓展可用
    void* pmr_extp;         //结构的拓展数据指针，以后拓展所用
    /*
     * 有时候内核要另起炉灶，不想把所有内存空间都交给内存管理器去做，因此要保留一部分内存空间 
     */
}phymmarge_t;
