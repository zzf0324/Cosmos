...
PUBLIC LKINIT void init_dftgraph(){
    ...
    kghp->gh_fnthight = 20;
    kghp->gh_linesz = 20 + 4;
    kghp->gh_deffontpx = BGRA(128, 0, 0);
    return;
}
...
void fill_fullbitmap(char_t *fname){
    ...
    machbstart_t *kmbsp = &kmachbsp;
    u64_t fadr = 0, fsz = 0;
    get_file_rvadrandsz(fname, kmbsp, &fadr, &fsz);
    if (0 == fadr || 0 == fsz){
        system_error("not bit file err\n");
    }
    bmdbgr_t *bmdp;
...
void hal_logo(){
    ...
    if (0 == fadr || 0 == fsz){
        system_error("logo loading err");
    }
    bmdbgr_t *bmdp;
    u64_t img = fadr + sizeof(bmfhead_t) + sizeof(bitminfo_t);
    bmdp = (bmdbgr_t *)((uint_t)img);
    pixl_t pix;
    for (int j = 554-1,k = 0; j >= 214; j--){
        for (int i = 342; i < 682; i++){
            pix = BGRA(bmdp[k].bmd_r, bmdp[k].bmd_g, bmdp[k].bmd_b);     //从位图中取出一个像素点
            drxw_pixcolor(kghp, i, j, pix);      //在kghp对应的图形设备上的i,j位置绘制像素点
            k++;    //处理下一个像素
        }
    }
    hint("logo加载完毕\n");
